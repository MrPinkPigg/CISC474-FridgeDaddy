	...
	methods: {
		print() {
			$("#printContainer").jqprint({
				debug:false,//Show iframe effect Default is false
				importCSS:true,//true introduces the page's css (if true, it will first look for $("link[media=print]"), if not it will go to the css file in $("link"))
				printContainer:true,//Indicates if the originally selected object must be included in printing (setting it to false may break your CSS rules).
				operaSupport:false//Support opera browser
			});
		},
		// Return to previous 
		rollBackBtn() {
